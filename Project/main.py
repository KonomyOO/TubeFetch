...
class YouTubeDownloaderApp:
    ...
    def _fetch_streams(self):
        ...
        try:
            ...
            video_streams = [f for f in video_info['formats'] if f.get('vcodec') != 'none' and f.get('ext') == 'mp4' and f.get('format_id') != '18']
            ...
            for stream in sorted_best_streams:
                ...
                print(f"DEBUG: Combined Stream - format_id: {stream.get('format_id')}, height: {height}, filesize: {stream.get('filesize')}, filesize_approx: {stream.get('filesize_approx')}") # ADD THIS LINE
            ...
            for stream in true_video_only_streams:
                ...
                if height:
                    ...
                    if current_best is None or \
                       (stream.get('filesize') or current_best.get('filesize_approx', 0)) > \
                       (current_best.get('filesize') or current_best.get('filesize_approx', 0)):
            ...
            for stream in sorted_best_video_only_streams:
                height = stream.get('height', 'N/A')
                quality_label = f"{height}p Video Only (No Audio)"
                # Removed filesize display for video-only streams due to potential inaccuracies

                print(f"DEBUG: Video-Only Stream - format_id: {stream.get('format_id')}, height: {height}, filesize: {stream.get('filesize')}, filesize_approx: {stream.get('filesize_approx')}") # ADD THIS LINE
                # Store a format string that explicitly requests video-only for this height
                # This is the key change: use a selector, not just format_id
                self.available_streams[quality_label] = f"bestvideo[height={height}][ext=mp4]"
                quality_options.append(ft.dropdown.Option(quality_label))
    ...
    def _download_video(self):
        ...
        try:
            ...
            if quality == "MP3 Audio Only":
                ...
                ydl_opts['format'] = format_id # Use the specific format_id selected by the user
                # No postprocessors needed if we're directly requesting the MP4 video-only stream
            else:
